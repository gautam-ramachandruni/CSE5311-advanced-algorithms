...
class Treap
{
  public:
    ...
    Treap( const Treap & rhs , int x_coord )
    {
        nullNode = new TreapNode;
        nullNode->left = nullNode->right = nullNode;
        nullNode->priority = INT_MAX;
    ...
    void insert( const Comparable & x , int x_coord )
    {
        insert( x, x_coord, root );
    }

    int calcDist( int a, int b, int c )
    {
        if( a > b )
            return c = ( ( ( a * a ) + ( b * b ) )^1/2 );
        else if( b > a )
            return c = ( ( ( b * b ) + ( a * a ) )^1/2 );
        else
            return c = 0;
    }
  ...
  private:
    struct TreapNode
    {
        ...
        TreapNode( const Comparable & theElement,int x_coord, TreapNode *lt, TreapNode *rt, int pr )
     ...
     */
    void insert( const Comparable & x, int x_coord, TreapNode* & t )
    {
        if( t == nullNode )
            t = new TreapNode( x, x_coord, nullNode, nullNode, randomNums.randomInt( ) );
        else if( x < t->element )
        {
            insert( x, x_coord, t->left );
            if( t->left->priority < t->priority )
                rotateWithLeftChild( t );
        }
        else if( t->element < x )
        {
            insert( x, x_coord, t->right );
